...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".sysmon__load{padding-left:44px;color:#6b7484;font-size:10px;margin:0 0 7px;font-variant-numeric:tabular-nums}",
		...
		function Body(props) {
			...
			if (d.swap) {
				children.push(jsx(Meter, {
					label: "SWAP", value: d.swap.pct, text: pctText(d.swap.pct), color: "#6c9bd1",
					sub: fmtSize(d.swap.usedMB) + " / " + fmtSize(d.swap.totalMB) + " used"
				}));
			}
			// Load average (1 . 5 . 15 min) + uptime, derived from the `os` module.
			if (d.load) {
				const l = d.load;
				children.push(jsx("div", {
					className: "sysmon__load",
					children: "load " + l["1"] + " " + l["5"] + " " + l["15"] +
						(d.uptimeSec ? "  \u00b7  up " + fmtUptime(d.uptimeSec) : "")
				}));
			}
			if (d.disk) {
				children.push(jsx(Meter, {
					label: "DISK", value: d.disk.pct, text: pctText(d.disk.pct), color: "#e5c07b",
					sub: fmtSize(d.disk.usedMB) + " / " + fmtSize(d.disk.totalMB) + " used"
				}));
			} else {
				children.push(jsx(Meter, { label: "DISK", value: 0, text: "n/a", color: "#5b6472", sub: "not available on this host" }));
			}
			if (d.net) {
			...
			return jsx("div", {
				className: "sysmon__body",
				children: jsxs(Fragment, { children: children })
			});
		}
		...
		function Monitor() {
			...
			// CSS lives for the whole mount so the collapsed frame still
			// renders correctly when reopened — inject exactly once.
			useEffect(() => {
			...
			// Only the polling timer toggles with `open`.
			useEffect(() => {
				if (!open) return; // collapsed: no polling until reopened
				const FETCH_TIMEOUT_MS = 2500; // longer than the 2s poll; resets each request
				let alive = true;
				// Track the in-flight request so we can abort it on teardown.
				let current = null;
				const timer = setInterval(tick, POLL_MS);

				function tick() {
					// A fresh AbortController per request: a timed-out request must
					// never poison the next one's signal.
					const controller = new AbortController();
					current = controller;
					const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
					fetch(STATS_PATH, { cache: "no-store", signal: controller.signal })
						.then((r) => r.json())
						.then((body) => { if (alive) setData(body); })
						.catch((e) => { if (alive && e.name !== "AbortError") setData({ error: String((e && e.message) || e) }); })
						.finally(() => { if (current === controller) current = null; clearTimeout(timeout); });
				}
				tick(); // grab an immediate sample on (re)open

				return () => {
					alive = false;
					clearInterval(timer);
					if (current) current.abort(); // cancel any in-flight request on collapse
				};
			}, [open]);
