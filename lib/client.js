// dsh-sysmon — browser half (persistent profile bundle).
//
// A floating bashtop-style panel pinned to the top-right of the dsh web GUI,
// registered into the frame-wide `shell.overlay` slot. It polls the host
// route `/api/sysmon-stats` every 2 seconds and renders CPU / RAM / DISK /
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const POLL_MS = 2000;
		const STATS_PATH = "/api/sysmon-stats";
		...
		function Monitor() {
			...
			useEffect(() => {
				...
				style.setAttribute("data-plugin", "dsh-sysmon");
