...
// ("cat" is no longer needed since /proc reads go through fs.readFileSync.)
const SELF = new Set([
...
// subprocess needed. Keeps the host half mostly fork-free (only ps/df spawn).
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          // Never leak the underlying command/error string to the client — it
          // may contain an absolute path or tooling detail. Log the detail
          // server-side only.
